        ...
        try:
            ...
            try:
                ...
                if avail0 <= 0:
                    ...
                    return False
                # 用可用余额倒推出最大可下名义与数量：notional_max = avail / (1/L + fee + safety)
                denom = (1.0 / max(1.0, leverage)) + taker_fee + safety
                if denom <= 0:
                    denom = (1.0 / max(1.0, leverage)) + taker_fee + safety
                notional_max = avail0 / denom
                qty_max_raw = max(0.0, notional_max / max(1e-12, price_for_notional))
                qty_max = qty_max_raw
                # 向下对齐步进
                try:
                    if lot_sz and float(lot_sz) > 0:
                        stepf = float(lot_sz)
                        qty_max = float(int(qty_max / stepf) * stepf)
                except Exception:
                    pass
                # 若对齐后低于最小单位，尝试最小单位是否可承受，否则放弃
                if qty_max <= 0 or (min_amount > 0 and qty_max < min_amount):
                    notional_min, required_min = _required_for(min_amount)
                    if avail0 >= required_min:
                        qty_max = float(min_amount)
                    else:
                        logger.warning(f"⚠️ 余额倒推出的最大数量<最小单位且不足以支付最小单位，跳过 {symbol}: avail={avail0:.4f}U need_min={required_min:.4f}U notional_max={notional_max:.4f}U")
                        return False
                # 最终使用策略数量与余额可承受上限取较小值
                if contract_size > qty_max:
                    logger.info(f"🧮 余额倒推: avail={avail0:.4f}U L={leverage:.2f} p={price_for_notional:.6f} f={taker_fee} b={safety} => notional_max={notional_max:.4f}U qty_max_raw={qty_max_raw:.8f} 对齐→{qty_max:.8f}; 数量 {contract_size:.8f} → {qty_max:.8f}")
                    contract_size = float(round(qty_max, amount_precision))
