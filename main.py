    ...
    try:
        ...
        try:
            ...
            if ohlcv_confirm:
                ...
                macd_c, sig_c, _ = calculate_macd(df_confirm['close'], fast=MACD_FAST, slow=MACD_SLOW, signal=MACD_SIGNAL)
                ...
                if pos.get('strategy_type') == 'bb_rsi_1m':
                    ...
                    if ohlcv_1m:
                        ...
                        if df1m is not None and len(df1m) >= 30 and 'BB_mid' in df1m.columns:
                            ...
                            # 高频止盈：价格达到中轨1-2%范围
                            tp_hit = False
                            if side == 'long' and last_close_1m >= bb_mid_1m * 1.01:
                                tp_hit = True
                            elif side == 'short' and last_close_1m <= bb_mid_1m * 0.99:
                                tp_hit = True
                                
                            if mid_revert or rsi_revert or force_time or tp_hit:
                                reason = "BB中轨0.3%回归" if mid_revert else ("RSI极值反转" if rsi_revert else ("超过3分钟强制平" if force_time else "高频止盈触发"))
