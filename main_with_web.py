...
# HTML模板（与railway_dashboard.py相同）
HTML_TEMPLATE = '''
<!DOCTYPE html>
<html lang="zh-CN">
<head>
...
def start_web_server():
    ...
    print(f"🌐 启动Web监控界面，端口: {port}")
    app.run(host='0.0.0.0', port=port, debug=False, threaded=True)

def start_trading_system():
    """启动交易系统（这里需要集成main.py的交易逻辑）"""
    print("🚀 启动交易系统...")
    ...
    while True:
        try:
            ...
            # 更新仪表板数据
            update_dashboard_data(account_data, stats_data, positions_data, signals_data)
            
            print(f"📊 数据已更新: {datetime.now().strftime('%H:%M:%S')}")
